    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                QBCore.Functions.Progressbar("use_nos", "Installing NOS", 6000, false, true, { -- time is default 1000
                    ...
                    TriggerServerEvent("QBCore:Server:RemoveItem", 'nitrous', 1) -- 1
        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                    if IsControlJustPressed(0, 36) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then -- keylocation 36 = L+Ctrl
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level > 0 then -- - 1 ~= 0
                                    -- QBCore.Functions.Notify(VehicleNitrous[Plate].level, "error")
                                    if VehicleNitrous[Plate].level == VehicleNitrous[Plate].level then
                                        VehicleNitrous[Plate].level = (VehicleNitrous[Plate].level - 1)
                                    end
                                    TriggerServerEvent('nitrous:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - 1)) -- (VehicleNitrous[Plate].level - 1)
                                    TriggerEvent('hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
                                else
                                    --QBCore.Functions.Notify("BROKE", "error")
                                    ...
                                    SetVehicleEngineTorqueMultiplier(CurrentVehicle, 1.0) -- 1.0
                    ...
                    if IsControlJustReleased(0, 36) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then -- (0, 36)
